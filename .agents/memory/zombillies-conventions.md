...
- Round structure: rounds progress rightward (`roundStart` += ROUND_LEN per wave); midway checkpoint = full heal once per round; wave advances only after all zombies dead AND endpoint flag reached. Boss every 3rd wave (>1). Mic mode kills zombies on contact.
